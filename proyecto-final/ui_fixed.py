...
class VectorEntryFrame(ttk.Frame):
    ...
    def get_vector(self):
        ...
        try:
            ...
            vector = [Fraction(entry.get()) for entry in self.entries]
            return vector
        except ValueError:
            messagebox.showerror("Error", f"Todas las componentes del {self.label_text} deben ser números o fracciones (ej: '1/2').")
            return None
...
class MatrixEntryFrame(ttk.Frame):
    ...
    def get_matrix(self):
        ...
        try:
            from fractions import Fraction
            matrix = [[Fraction(entry.get()) for entry in row] for row in self.entries]
            return matrix
        except ValueError:
            messagebox.showerror("Error", f"Todos los elementos de la {self.label_text} deben ser números o fracciones (ej: '1/2').")
            return None
...
class ScalarEntryFrame(ttk.Frame):
    ...
    def get_value(self):
        ...
        try:
            from fractions import Fraction
            return Fraction(self.entry.get())
        except ValueError:
            messagebox.showerror("Error", f"El {self.label_text} debe ser un número o una fracción (ej: '1/2').")
            return None
                ...
                if method_name in ["es_linealmente_independiente", "es_combinacion_lineal"]:
                    ...
                    if tipo == "unica":
                        ...
                        for i, val in enumerate(solucion):
                            # Si es un objeto Fraction, formatearlo como a/b o como entero si es posible
                            if hasattr(val, 'numerator') and hasattr(val, 'denominator'):
                                if val.denominator == 1:
                                    result_str += f"x{i+1} = {val.numerator}\n"
                                else:
                                    result_str += f"x{i+1} = {val.numerator}/{val.denominator}\n"
                            else:
                    ...
                    if isinstance(result, list):
                        if isinstance(result[0], list):
                            ...
                            for j, val in enumerate(result):
                                ...
                                if hasattr(val, 'numerator') and hasattr(val, 'denominator'):
                                    if val.denominator == 1:
                                        vec_str += f"{val.numerator}"
                                    else:
                                        vec_str += f"{val.numerator}/{val.denominator}"
                                else:
                                    vec_str += f"{val}"
                        ...
                        if hasattr(result, 'numerator') and hasattr(result, 'denominator'):
                            if result.denominator == 1:
                                result_str = f"Resultado: {result.numerator}"
                            else:
                                result_str = f"Resultado: {result.numerator}/{result.denominator}"
                        else:
